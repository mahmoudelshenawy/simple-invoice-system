...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">الكتالوج</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ قائمة الخدمات</span>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
...
@endif
									...
									<table class="table key-buttons text-md-nowrap" data-page-length='50'style="text-align: center" id="example1">
										<thead>
											<tr>
												...
												<th class="wd-lg-20p"><span>الخدمة</span></th>
										...
										<tbody>
											...
											<tr>
												...
												<td>
                                                    <img src="{{ url($service->imagePath) }}" alt="" width="40" height="30" class="img-fluid">
													<br>
												</td>
												<td>
													{{$service->name}}
												...
												<td>
													<a href="/catalog/services/{{$service->id}}" class="btn btn-sm btn-primary">
														<i class="las la-search"></i>
													</a>
													<a href="/catalog/services/{{$service->id}}/edit" class="btn btn-sm btn-info">
														<i class="las la-pen"></i>
													</a>
													<a  href="#deleteserviceModal" class="modal-effect btn btn-sm btn-danger" data-effect="effect-scale"
													data-id="{{ $service->id }}" data-service_name="{{ $service->name }}"
       ...
       <div class="modal-dialog" role="document">
           <div class="modal-content">
               ...
               <form action="/catalog/services/destroy" method="post">
                   {{ method_field('delete') }}
                   {{ csrf_field() }}
                   <div class="modal-body">
                       <p>هل انت متاكد من عملية الحذف ؟</p><br>
                       <input type="hidden" name="id" id="serv_id" value="">
                       <input class="form-control" name="service_name" id="service_name_delete" type="text" readonly>
                   </div>
...
@section('js')
 ...
 <script>
	$('#deleteserviceModal').on('show.bs.modal', function(event) {
		...
		var modal = $(this)
		modal.find('.modal-body #serv_id').val(id);
		modal.find('.modal-body #service_name_delete').val(name);
	})
