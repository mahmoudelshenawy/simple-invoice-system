...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto"> <a href="/invoices" class="content-title">قائمة الفواتير</a>
                </h4>
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ إضافة فاتورة جديده</span>
    ...
    <div class="row">
        ...
        <div class="col-xl-12">
            <div class="card mg-b-20">
                <div class="card-body">
                    <form action="{{ url("/add_items_to_invoice/$invoice->id") }}" method="POST" autocomplete="off">
                        @csrf
                        {{-- 1 --}}
                        <div class="row">
                                        ...
                                        <div class="card-body">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <div class="custom-checkbox custom-control">
                                                    <input type="checkbox" data-checkboxes="mygroup" data-id = "{{$pro->id}}" class="custom-control-input" id="product_{{$pro->id}}" name="product_id[]" value="{{$pro->id}}" @if ($invoice->products->contains($pro)) checked @endif>
                                                    <label for="product_{{$pro->id}}" class="custom-control-label mt-1"></label>
                                                </div>
                                            </div>
                                            <div class="my-2">
                                               
                                                @if ($invoice->products->contains($pro))
                                               @php
                                                   $pro_invoice = $invoice->products->find($pro)
                                               @endphp
                                                <input type="number" name="quantity[]" id="quantity_{{$pro->id}}" class="form-control form-control-sm" value="{{$pro_invoice->pivot->quantity}}">
                                                @else
                                    ...
                                    <div class="card" style="min-height: 300px;">
                                        ...
                                        <div class="card-body">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <div class="custom-checkbox custom-control">
                                                    <input type="checkbox" data-checkboxes="mygroup" data-id = "{{$serv->id}}" class="custom-control-input" id="service_{{$serv->id}}" name="service_id[]" value="{{$serv->id}}" @if ($invoice->services->contains($serv)) checked @endif>
                                                    <label for="service_{{$serv->id}}" class="custom-control-label mt-1"></label>
                                                </div>
                                            </div>
                                            <div class="my-2">
                                               
                                                @if ($invoice->services->contains($serv))
                                               @php
                                                   $serv_invoice = $invoice->services->find($serv)
                                               @endphp
                                                <input type="number" name="quantity_service[]" id="quantity_serve_{{$serv->id}}" class="form-control form-control-sm" value="{{$serv_invoice->pivot->quantity}}">
                                                @else
